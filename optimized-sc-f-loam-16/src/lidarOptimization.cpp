...
EdgeAnalyticCostFunction::EdgeAnalyticCostFunction(Eigen::Vector3d curr_point_, Eigen::Vector3d last_point_a_, Eigen::Vector3d last_point_b_)
    ...
    /*curr_point_ last_point_a_ last_point_b_这三个参数是传入的已知参数
     * 其中curr_point_是雷达坐标系下的当前帧边界点
     * last_point_a_，last_point_b_是与当前边界点最近的直线点
     */
...
{
    /*
     * parameters是待优化的参数,7x1
     * 其中前四个是旋转参数（四元数）
     * 后三个是平移参数（x,y,z）
     * residuals 是一个1x1的参数：物理意义为点到线的距离
    */
    Eigen::Map<const Eigen::Quaterniond> q_last_curr(parameters[0]);
    Eigen::Map<const Eigen::Vector3d> t_last_curr(parameters[0] + 4);
    Eigen::Vector3d lp;//lp作为利用待优化参数转换到Map坐标系下的位姿
    lp = q_last_curr * curr_point + t_last_curr; 

    Eigen::Vector3d nu = (lp - last_point_a).cross(lp - last_point_b);//.cross代表叉乘
    Eigen::Vector3d de = last_point_a - last_point_b;//直线的方向向量
    double de_norm = de.norm();//求模
    /*
     * 这里的nu.norm()可以表示为以alp，blp为边的平行四边形的面积；de.norm()表示ab之间的距离
     * nu.norm()/de_norm就是用四边形的面积除以ab，得到lp到ab之间的垂直距离，即点到线的距离
    */
...
bool SurfNormAnalyticCostFunction::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    residuals[0] = plane_unit_norm.dot(point_w) + negative_OA_dot_norm;//.dot代表点乘
...
{
    //x代表优化前的参数，前四个为四元数表示旋转，后三个为位移参数x，y，z
