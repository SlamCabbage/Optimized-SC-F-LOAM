...
//            std::cout << "滤波后的点数量" << downsampledSurfCloud->size() << std::endl;
            laserLoopOptimization.save_and_update_PGnode(thisKeyFrame, thisKeyFrame_surf, thisKeyFrame_edge, pose_curr, timeLaserOdometry, timeLaser, timeGroud_truth);
...
void process_viz_map()
{
	...
	if(laserCloudMapPGO->size() == 0){
        ...
        pass.setInputCloud(laserCloudMapPGO);
        pass.setFilterFieldName("z");
        pass.setFilterLimitsNegative(false);
        pass.setFilterLimits(-5, 10);
        pass.filter(*laserCloudMapPGO);
...
int main(int argc, char **argv)
{
    ...
    laserLoopOptimization.save_directory = "/home/llz/ourdataset/message/";
    ...
    nh.param<double>("keyframe_meter_gap", laserLoopOptimization.keyframeMeterGap, 1); // pose assignment every k m move
    nh.param<double>("keyframe_deg_gap", laserLoopOptimization.keyframeDegGap, 5); // pose assignment every k deg rot
    laserLoopOptimization.keyframeRadGap = deg2rad(laserLoopOptimization.keyframeDegGap);

    nh.param<double>("sc_dist_thres", laserLoopOptimization.scDistThres, 0.3);
