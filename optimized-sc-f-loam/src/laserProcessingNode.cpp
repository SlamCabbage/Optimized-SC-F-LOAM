...
void laser_processing() {
    while (1) {
        //由于只处理点云数据，因此不需要进行时间同步
        if (!pointCloudBuf.empty()) {
            //read data
            //处理pointCloudBuf时，需要线程锁，以免同一个内存同时被多次操作
            mutex_lock.lock();
            //以下4行为：取出一帧点云数据
            pcl::PointCloud<pcl::PointXYZI>::Ptr pointcloud_in(new pcl::PointCloud<pcl::PointXYZI>());
            pcl::fromROSMsg(*pointCloudBuf.front(), *pointcloud_in);
            ros::Time pointcloud_time = (pointCloudBuf.front())->header.stamp;
            pointCloudBuf.pop();//queue
            mutex_lock.unlock();

            //定义两个点云容器，用于储存边界点和平面点
            ...
            /**
             * 特征提取的关键函数laserProcessing.featureExtraction
             */
...
//            ROS_INFO("average laser processing time %f ms \n \n", total_time/total_frame);
            ...
            //pointcloud_filtered是当前帧边界点和平面点的集合
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "sc_floam_laser_processing_node");
    ros::NodeHandle nh;
